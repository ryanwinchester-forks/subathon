defmodule SubathonWeb.ProfileAuth do
  ...
  # the token expiry itself in ProfileToken.
  @max_age 60 * 60 * 24 * 60
  ...
  """
  def login_profile(conn, profile, params \\ %{}) do
  ...
  def fetch_current_profile(conn, _opts) do
    ...
    profile = profile_token && Accounts.get_profile_by_session_token(profile_token)
  ...
  ## `on_mount` arguments
    ...
    * `:mount_current_profile` - Assigns current_profile
      to socket assigns based on profile_token, or nil if
      there's no profile_token or no matching profile.

    * `:ensure_authenticated` - Authenticates the profile from the session,
      and assigns the current_profile to socket assigns based
      on profile_token.
      Redirects to login page if there's no logged profile.

    * `:redirect_if_profile_is_authenticated` - Authenticates the profile from the session.
      Redirects to signed_in_path if there's a logged profile.
  ...
  the current_profile:
      ...
      defmodule SubathonWeb.PageLive do
        ...
        on_mount {SubathonWeb.ProfileAuth, :mount_current_profile}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{SubathonWeb.ProfileAuth, :ensure_authenticated}] do
  ...
  defp mount_current_profile(socket, session) do
    Phoenix.Component.assign_new(socket, :current_profile, fn ->
      if profile_token = session["profile_token"] do
        Accounts.get_profile_by_session_token(profile_token)
      end
    end)
  end
